...
const CaretakerHomeComponent = ({ userDetails }: { userDetails: User }) => {
  ...
  const hasBlindUsers = userDetails.connectedUsers && userDetails.connectedUsers.length > 0;
    ...
    >
      ...
      <View className="px-5 mt-6">
        ...
        <View className="bg-white rounded-xl p-5 shadow-sm mb-6">
          <Text className="text-gray-500 text-sm mb-1">PROFILE INFORMATION</Text>
          ...
          <View className="flex-row items-center py-3 border-b border-gray-100">
            ...
            <View>
              ...
              <Text className="text-gray-800">{userDetails.fullName || 'Not provided'}</Text>
            </View>
          </View>
          
          <View className="flex-row items-center py-3 border-b border-gray-100">
            <Ionicons name="mail-outline" size={20} color="#4b5563" className="mr-3" />
            <View>
              <Text className="text-gray-500 text-xs mb-1">Email Address</Text>
              <Text className="text-gray-800">{userDetails.email || 'Not provided'}</Text>
            </View>
          </View>
          
          <View className="flex-row items-center py-3">
            <Ionicons name="call-outline" size={20} color="#4b5563" className="mr-3" />
            <View>
              <Text className="text-gray-500 text-xs mb-1">Phone Number</Text>
              <Text className="text-gray-800">{userDetails.phone || 'Not provided'}</Text>
            </View>
          </View>
        </View>
        ...
        <View className="bg-white rounded-xl p-5 shadow-sm">
          ...
          <View className="flex-row border-b border-gray-200 mb-3">
            ...
            >
              <Text className={selectedTab === 'all' ? 'text-indigo-600 font-medium' : 'text-gray-500'}>All</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => setSelectedTab('active')}
              className={`py-2 px-4 ${selectedTab === 'active' ? 'border-b-2 border-indigo-500' : ''}`}
            >
              <Text className={selectedTab === 'active' ? 'text-indigo-600 font-medium' : 'text-gray-500'}>Active</Text>
            </TouchableOpacity>
            <TouchableOpacity 
              onPress={() => setSelectedTab('inactive')}
              className={`py-2 px-4 ${selectedTab === 'inactive' ? 'border-b-2 border-indigo-500' : ''}`}
            >
              <Text className={selectedTab === 'inactive' ? 'text-indigo-600 font-medium' : 'text-gray-500'}>Inactive</Text>
            </TouchableOpacity>
          </View>
              ...
              return (
                <TouchableOpacity 
                  ...
                  className={`flex-row justify-between items-center py-3 ${
                    index < userDetails.connectedUsers!.length - 1 ? 'border-b border-gray-100' : ''
                  }`}
                  activeOpacity={0.7}
                >
                  <View className="flex-row items-center">
                    ...
                    <View>
                      <Text className="text-gray-800 font-medium">{blindUser.fullName || 'Unknown'}</Text>
                      <Text className="text-gray-500 text-xs">{blindUser.email}</Text>
                    </View>
                  </View>
                  <View className="flex-row items-center">
                    <View className={`h-2.5 w-2.5 rounded-full ${status === 'active' ? 'bg-green-500' : 'bg-gray-300'} mr-2`} />
                    <TouchableOpacity className="p-2">
                      <Ionicons name="locate-outline" size={20} color="#6366f1" />
                    </TouchableOpacity>
                  </View>
                </TouchableOpacity>
              );
        ...
        <View className="bg-white rounded-xl p-5 shadow-sm mt-6">
          ...
          <View className="flex-row justify-between">
            <TouchableOpacity className="items-center w-[30%]" activeOpacity={0.7}>
              ...
              <Text className="text-gray-800 text-xs text-center">Emergency Alert</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity className="items-center w-[30%]" activeOpacity={0.7}>
              ...
              <Text className="text-gray-800 text-xs text-center">Track Location</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity className="items-center w-[30%]" activeOpacity={0.7}>
              ...
              <Text className="text-gray-800 text-xs text-center">Quick Call</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
